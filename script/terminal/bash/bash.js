class Bash {
  constructor(options) {
    ...
    this.vfs = new VirtualFS({ user: "kamil", fs: options.json });
    this.history = [];
  ...
  appendNewPointerResult(path, input, output) {
    ...
    const out = doc.body.firstElementChild;
    ...
    if (target.Children.length > 7) target.removeChild(target.Children[0]);
  }
  ...
  clearTerminal() {
    ...
    if (target.Children.length > 2)
      [...target.Children]
        .slice(0, target.Children.length - 2)
        .forEach((child) => target.removeChild(child));
    else return;
  }

  fetchHelpData = (command) =>
    this.commands[command].help ? this.commands[command].help : "404";
  ...
  async printCommandHelp(command, args) {
    ...
    this.appendNewPointerResult(
      this.vfs.formatPath(this.vfs.pathArrayToString(this.vfs.cwd)),
      command + " " + args,
      div.outerHTML
    );
  }
  ...
  updateCurrentPath = () => (
    (document.getElementById("cwd").innerText = this.vfs.formatPath(
      this.vfs.pathArrayToString(this.vfs.cwd)
    )),
    (document.querySelector(".terminal-title").innerText =
      "kamil@terminal: " +
      this.vfs.formatPath(this.vfs.pathArrayToString(this.vfs.cwd)))
  );
  ...
  throwError(errorType, command, args) {
    switch (errorType) {
      case "readonly":
        ...
        if (command === "touch")
          p.innerText = `${command}: cannot touch '${args}': Permission denied`;
        if (command === "mv")
          p.innerText = `${command}: cannot stat '${args}': Permission denied`;
      ...
      case "badarg":
        var p = document.createElement("p");
        p.innerHTML = `${command}: invalid option -- '${args
          .toString()
          .replaceAll(
            "-",
            ""
          )}'<br> Try '${command} --help' for more information.`;
        this.appendNewPointerResult(
          this.vfs.formatPath(this.vfs.pathArrayToString(this.vfs.cwd)),
          command + (args ? ` ${args}` : ""),
          p.outerHTML
  ...
  cmd_cd(args, command) {
    ...
    if (args[0] === "-") {
      ...
      var p = document.createElement("p");
      this.appendNewPointerResult(
        this.vfs.formatPath(this.vfs.pathArrayToString(temp)),
        command + ` ${args}`,
        p.outerHTML
      );
      return;
    ...
    targetNode = this.vfs._getNodeByPathArray(
      this.vfs.resolvePath(resolvedPath)
    );
    if (!targetNode) {
  ...
  cmd_ls(args, command) {
    ...
    let Children = Object.Children(targetNode.Children || {}).map(
      ([name, node]) => {
        return { name, ...node };
      }
    );
    ...
    if (!options.all) {
      Children = Children.filter((e) => !e.name.startsWith("."));
    }
    ...
    if (options.long) {
      ...
      //pTotal.innerText = 'total ' + Object.keys(this.vfs._getNodeByPathArray(this.vfs.cwd).Children).length;
      pTotal.innerText =
        "total " +
        (options.all
          ? Object.keys(this.vfs._getNodeByPathArray(this.vfs.cwd).Children)
              .length
          : Object.keys(
              this.vfs._getNodeByPathArray(this.vfs.cwd).Children
      ...
      if (options.all) {
        ...
        size2.innerText = Object.keys(
          this.vfs._getNodeByPathArray(this.vfs.cwd.slice(0, -1)).Children
        ).length;
      ...
      Children.forEach((e) => {
        ...
        const userPerm = document.createElement("p");
        userPerm.classList.add("user-permission");
        userPerm.innerText = e.userpermission
          ? e.userpermission
          : "kamil users  ";
        li.appendChild(userPerm);
        ...
        if (e.interactible || false) {
          ...
          a.classList.add(
            "project-name",
            ...(e.type === "dir" ? ["folder-type"] : [])
          );
          a.innerText = e.name;
          a.href = e.href ? e.href : "";
          li.appendChild(a);
        } else {
          // Dziwne układy kodu takie jak te to wina nowego formattera w moim IDE
  ...
  cmd_cp(args, command) {
    ...
    sourceNode = this.vfs._getNodeByPathArray(
      this.vfs.resolvePath(resolvedSource)
    );
    if (!sourceNode) {
  ...
  cmd_rmdir(args, command) {
    ...
    targetNode = this.vfs._getNodeByPathArray(
      this.vfs.resolvePath(resolvedTarget)
    );
...
class VirtualFS {
  constructor(options) {
    ...
    this.rootDir = options.fs; // this.rootDir defniował obiekt w formacie JSON przed reimplementacją.
    // JSON który miał reprezentować system plików to teraz fs.json w folderze terminal
    // Próba wprowadzenia już zfetchowanej zawartości pliku przed instancjowaniem klasy Bash nie powiodła się
    this.cwd = ["/", "home", user];
    this.home = ["/", "home", user];
  ...
  formatPath = (pathString) =>
    ((prefix) =>
      pathString.startsWith(prefix)
        ? pathString.replace(prefix, "~")
        : pathString)(this.pathArrayToString(this.home));
  ...
  resolvePath(pathString) {
    ...
    if (pathString.startsWith("~")) {
      ...
      pathString =
        homeStr + (trail ? (trail.startsWith("/") ? "" : "/") + trail : "");
    }
  ...
  changeCwd(pathString) {
    ...
    if (!node)
      throw new Error("No such directory: " + this.pathArrayToString(arr));
    if (node.type !== "dir")
      throw new Error("Not a directory: " + this.pathArrayToString(arr));
    this.cwd = arr;
    return this.cwd;
  }
}
