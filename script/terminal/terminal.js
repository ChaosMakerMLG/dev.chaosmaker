...
if(isMobileByUA) {
  document.getElementById('hiddenInput').addEventListener('beforeinput', inputHandler);
}
...
function inputHandler(event) {
  ...
  switch(key) {
    ...
    case "ArrowRight":
      ...
      break;
    case "ArrowLeft":
      if(string.innerText.length === 0 || charIndex === (string.innerText.length + 1)) break;
      MovePointer('move', 'fwd', '0');
      break;
    case "ArrowUp":
      if(historyIndex < bash.history.length && bash.history[historyIndex + 1]) historyIndex++;
        if(string.innerText.length > 0 && !bash.history.includes(string.innerText)) tempHistory = string.innerText;
        bash.history[historyIndex] ? RenderString(bash.history[historyIndex], true, string) : '';
        charAdded = true;
      break;
    case "ArrowDown":
      ...
      historyIndex--;
      bash.history[historyIndex] ? RenderString(bash.history[historyIndex], true, string) : 
      (tempHistory ? RenderString(tempHistory, true, string) : string.replaceChildren());
      charAdded = true;
      break;
    default: 
      if (!event.repeat && key.length === 1 && string.innerText.length < maxLength) {
...
function TerminalShake(variant) {
  ...
  switch(variant) {
    case 'red':
      ...
      break;
    default:
      document.querySelector('.terminal-window').addEventListener('animationend', () => {
        document.querySelector('.terminal-window').style.animation = '';
      });
      document.querySelector('.terminal-window').style.animation = 'shake 0.82s cubic-bezier(.36,.07,.19,.97) both';
      isPlaying = false;
      break;
  }
}
...
async function UseCommand(command) {
    ...
    isPlaying = false;
    if(!haltCommand) dispatchEvent(new KeyboardEvent('keydown', {'key': 'Enter'}));
}
...
async function AnimatedRenderString(string, element) {
  ...
  for(let i = 0; i < string.length; i++) {
    ...
    isPlaying = false;
                ...
                /* set/move | fwd/bck | index */
function MovePointer(option, direction, n) {
  const arr = Array.from(document.querySelector('.pointer-wrapper').children).
  filter((elem) => elem.className === 'cursor');
  const chars = Array.from(document.getElementById('command-string').children).concat(arr);
  if(option === 'move') {
    //left
    if(direction === 'fwd') {
...
function RenderString(string, replace, element) {
  ...
  for(let i = 0; i < string.length; i++) {
    ...
    element.insertBefore(span, element.children[element.children.length - (charIndex - 1)]);
